...
export const createStringValidator = <TModel extends Record<string, any>>(
  ...
  _validator.hasMinLength = (minLength: number) => {
    validations[property].push(() => useLimitedLength(model, property, { limit: minLength, type: Extremes.minimum }));
    return _validator;
  };

  _validator.hasMaxLength = (maxLength: number) => {
    validations[property].push(() => useLimitedLength(model, property, { limit: maxLength, type: Extremes.maximum }));
    return _validator;
  };
